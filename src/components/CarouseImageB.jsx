...
export const CarouseImageB = ({ data }) => {
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="relative flex justify-center items-center w-[600px] h-[400px]">
        <ChevronLeft
          onClick={prevSlide}
          className="absolute left-4 w-8 h-8 text-white cursor-pointer"
        />
        <img
          src={data[slide].src}
          alt={data[slide].alt}
          className="absolute w-full h-full rounded-md shadow-md transition-opacity duration-500"
          style={{ opacity: 1 }}
        />
        <ChevronRight
          onClick={nextSlide}
          className="absolute right-4 w-8 h-8 text-white cursor-pointer"
        />
      </div>
